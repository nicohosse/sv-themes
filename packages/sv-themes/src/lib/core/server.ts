...
import { getThemeClass, type ThemeManager } from "./theme-manager.svelte.ts";
...
export function getSSRAttributes<Themes extends ThemesRecord>(themeManager: ThemeManager<Themes>) {
	...
	for (const attribute of themeManager.attributes) {
		const value = attribute === "class" ? getThemeClass(themeManager, resolvedTheme.id) : resolvedTheme.id;
		attributes[attribute] = value;
	}

	if (themeManager.themeForcedAttribute && themeManager.forcedTheme)
		attributes[themeManager.themeForcedAttribute] = "true";
